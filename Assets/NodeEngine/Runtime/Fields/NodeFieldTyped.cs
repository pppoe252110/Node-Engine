...
[Serializable]
public class NodeFieldTyped<T> : NodeFieldBase<T>, ITypeChangeable where T : IConnectorValue
{
    public delegate void ValueHandlerFunc(T value);
    public event ValueHandlerFunc CurrentValueHandler;

    public NodeFieldTyped()
    {
    ...
    private void InitializeDefaultValue()
    {
        ...
        else if (typeof(T) == typeof(ConnectorValueType))
            _currentValue = (T)(object)new ConnectorValueType(typeof(object));
        else if (typeof(T) == typeof(ConnectorValueObject))
            _currentValue = (T)(object)new ConnectorValueObject(null);
        else
            _currentValue = default;
    }

    public NodeFieldTyped<T> SetHandler(ValueHandlerFunc handler)
    {
        CurrentValueHandler = handler;
        return this;
    }

    public NodeFieldTyped<T> SetDefaultValue(T value)
    {
        _currentValue = value;
        return this;
    }
    ...
    private void UpdateCurrentValueTyped(IConnectorValue<T> newValue)
    {
        ...
        else if (_currentValue is ConnectorValueType typeConnector && innerValue is Type typeVal)
        {
            typeConnector.SetInnerValue(typeVal);
        }
    ...
    private void ProcessDataInput(Connector connectedConnector)
    {
        ...
        var connectedField = connectedConnector.Field as NodeFieldBase;
        ...
        var sourceValue = connectedField.GetCurrentValue();
    ...
    // ITypeChangeable implementation
    public bool ChangeOutputType(Type newType)
    {
        // Only ConnectorValueObject can change its inner type dynamically
        if (_currentValue is ConnectorValueObject connectorObject)
        {
            try
            {
                // Update the stored type in the connector value
                connectorObject.SetInnerValue(CreateDefaultValue(newType));
                return true;
            }
            catch (Exception e)
            {
                Debug.LogError($"Failed to update output type: {e.Message}");
                return false;
            }
        }

        // For other types, we need to check if they can be converted
        return TryConvertToType(newType);
    }

    public bool CanUpdateType(Type newType)
    {
        // Only ConnectorValueObject can freely change types
        if (_currentValue is ConnectorValueObject) return true;

        // Check if current type can be converted
        return TryConvertToType(newType);
    }

    public override bool TryUpdateOutputType(Type newType)
    {
        if (Connector == null) return false;

        // Update the current value's inner type and value immediately
        if (_currentValue is ConnectorValueObject connectorObject)
        {
        ...
        // Use the centralized TypeChangeService (this will now use the updated ValueType)
        return TypeChangeService.TryChangeConnectorType(Connector, newType);
    }
    ...
    private bool TryConvertToType(Type newType)
    {
        // Check if current type can be converted to new type
        Type currentType = _currentValue?.InnerType ?? typeof(object);

        if (currentType == newType) return true;
        if (newType == typeof(object)) return true;

        // Allow numeric conversions
        if ((currentType == typeof(int) && newType == typeof(float)) ||
            (currentType == typeof(float) && newType == typeof(int)))
        {
            return true;
        }
    ...
    // Helper method for nodes to trigger execution
    public void TriggerExecution()
    {
        // If this is connected to an executable node's input, trigger its execution
        if (Connector != null && Connector.Node is ExecutableNodeBase executableNode)
        {
            executableNode.Execute();
        }
    }
